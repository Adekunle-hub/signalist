...
function Command({
  ...
  return (
    <CommandPrimitive
      data-slot="command"
      className={cn(
        "bg-[#212328] shadow-[0_2px_30px_0_rgba(0,0,0,0.05)] text-popover-foreground flex h-full w-full flex-col overflow-hidden rounded-md border-0! ",
        className
      )}
      {...props}
    />
...
function CommandInput({ className, onClear, ...props }: CommandInputProps) {
  return (
    <div
      data-slot="command-input-wrapper"
      className="flex h-9 items-center gap-2   px-3"
    >
      <FiSearch className="size-3 shrink-0 text-white" />
      ...
      {onClear && (
        <FiX
          onClick={onClear}
          className="size-3 cursor-pointer shrink-0 text-[#9095A1]"
        />
      )}
    </div>
  );
